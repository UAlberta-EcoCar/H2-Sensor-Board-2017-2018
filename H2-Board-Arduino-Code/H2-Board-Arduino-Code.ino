...
void setup() {
  ...
  Serial.begin(115200); // Begin serial monitor output -- for testing and setting a value for h2_sensor_cutoff (remove later)
}
...
void loop() {
  ...
  buf.add(&h2_sensor_value);

  if(buf.numElements() < 10){
    return;
  }

  h2_sensor_avg = getBufAvg(buf);

  if ((estop1_value == LOW) || (estop2_value == LOW) || (h2_sensor_avg >= h2_sensor_cutoff))
  {
  ...
  if (h2_sensor_avg >= h2_sensor_cutoff) // If excess hydrogen is sensed, turn on H2 indicator LED
  {
  ...
  Serial.println(h2_sensor_avg); // Print read value from H2 sensor to serial monitor -- for testing and setting a suitable value for h2_sensor_cutoff (remove later)
  delay(50); // Delay reading to make serial monitor easier to track (remove later)

}

int getBufAvg(RingBufC buf){
  int avg = 0;
  int num_elements = buf.numElements();
